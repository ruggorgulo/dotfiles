...
(defconst dcl-keywords-regexp
  (regexp-opt
   '("if"                 "then"         "else"            "endif"
     "goto"               "gosub"        "return"
     "call"               "subroutine"   "endsubroutine")
   'symbols)
  "List of DCL block control keywords.")

(defconst dcl-f-functions-regexp
  (regexp-opt
  '("f$context"          "f$csid"       "f$cunits"
    "f$cvsi"             "f$cvtime"     "f$cvui"
    "f$delta_time"       "f$device"     "f$directory"
    "f$edit"             "f$element"    "f$environment"
    "f$extract"          "f$fao"        "f$fid_to_name"
    "f$file_attributes"  "f$getdvi"     "f$getenv"
    "f$getjpi"           "f$getqui"     "f$getsyi"
    "f$identifier"       "f$integer"    "f$length"
    "f$license"          "f$locate"     "f$match_wild"
    "f$message"          "f$mode"       "f$multipath"
    "f$parse"            "f$pid"        "f$privilege"
    "f$process"          "f$search"     "f$setprv"
    "f$string"           "f$time"       "f$trnlnm"
    "f$type"             "f$unique"     "f$user"
    "f$verify")
    'symbols)
  "List of DCL Lexical Functions.")

(defconst dcl-builtin-regexp
  (regexp-opt
   '("accounting"         "allocate"          "analyze"
     "append"             "assign"            "attach"
     "backup"             "call"              "cancel"
     "checksum"           "close"             "connect"
     "continue"           "convert"           "copy"
     "create"             "deallocate"        "deassign"
     "debug"              "deck"              "decrypt"
     "define"             "delete"            "deposit"
     "differences"        "directory"         "disable autostart"
     "disconnect"         "dismount"          "dump"
     "edit"               "enable autostart"  "encrypt"
     "eod"                "eoj"               "examine"
     "exchange"           "exchange"          "exit"
     "font"               "help"              "initialize"
     "inquire"            "install" ; "java" is not builtin
     "job"                "library"           "license"
     "link"               "login"             "logout"
     "macro"              "mail"              "merge"
     "message"            "mcr"               "monitor"           "mount"
     "ncs"                "on"                "open"
     "password"           "patch"             "phone"
     "pipe"               "pppd"              "print"
     "product"            "purge"             "read"
     "recall"             "rename"            "reply"
     "request"            "run"               "runoff"
     "search" ; "set" and "show" are handled separately (see below)
     "sort"               "spawn"             "start"
     "stop"               "submit"            "synchronize"
     "type"               "unlock"            "view"
     "wait"               "write"             "xauth")
   'symbols)
  "DCL keywords.")

(defconst dcl-set-subcommands-regexp
  (regexp-opt
   (mapcar '(lambda (s) (concat "set " s))
           '("accounting"         "audit"          "bootblock"
             "broadcast"          "cache"          "card_reader"
             "cluster"            "command"        "control"
             "cpu"                "day"            "default"
             "device"             "directory"      "display"
             "entry"              "file"           "host"
             "image"              "key"            "logins"
             "magtape"            "message"        "network"
             "on"                 "output_rate"    "password"
             "preferred_path"     "prefix"         "printer"
             "process"            "prompt"         "protection"
             "queue"              "restart_value"  "rights_list"
             "rms_default"        "root"           "security"
             "server acme_server" "server registry_server" "server security_server"
             "shadow"             "symbol"         "terminal"
             "time"               "verify"         "volume"
             "working_set" ))
   'symbols)
  "DCL subcommands for SET keyword.")

(defconst dcl-show-subcommands-regexp
  (regexp-opt
   (mapcar '(lambda (s) (concat "show " s))
           '("accounting"          "audit"         "broadcast"
             "cluster"             "cpu"           "default"
             "devices"             "display"       "entry"
             "error"               "fastpath"      "image"
             "intrusion"           "key"           "license"
             "logical"             "memory"        "network"
             "printer"             "process"       "protection"
             "queue"               "quota"         "rms_default"
             "root"                "security"      "server acme_server"
             "server registry_server"              "shadow"
             "status"              "symbol"        "system"
             "terminal"            "time"          "translation"
             "users"               "working_set"   "zone"))
   'symbols)
  "DCL subcommands for SHOW keyword.")

(defconst dcl-dot-operators-regexp
  (regexp-opt
   '(".eq." ".not." ".or." ".and." ".lt." ".gt." ".le." ".ge." ".eqs." ".nes.")
   'symbols)
   "DCL .OP. operators.")

(defconst dcl-global-symbols-regexp
  (regexp-opt
   '("$status" "$restart" "$severity" "$status"
     "sql$database"
     "sys$input" "sys$output" "sys$pipe" "sys$error" "sys$command")
   'symbols)
  "DCL predefined logicals.")

(defconst dcl-label-regexp
  "^\\$[ \t]*\\([a-z_][a-z0-9_]+\\):"
  "DCL label")

(defconst dcl-comment-regexp
  "\\_<!\\_>.*$"
  "Regexp string that matches a comment until the end of the line.")

(defconst dcl-special-symbol-regexp
  (regexp-opt
   '("@" "=" "==" ":=" ":==" "|" ">" "2>" "&" ";")
   'symbols)
  "DCL special symbols")

(defconst dcl-font-lock-keywords
  (list (list dcl-keywords-regexp                0 'font-lock-keyword-face)
        (list dcl-f-functions-regexp             0 'font-lock-function-name-face)
        (list dcl-dot-operators-regexp           0 'font-lock-type-face)
        (list dcl-builtin-regexp                 0 'font-lock-builtin-face)
        (list dcl-set-subcommands-regexp         0 'font-lock-builtin-face)
        (list dcl-show-subcommands-regexp        0 'font-lock-builtin-face)
        (list dcl-global-symbols-regexp          0 'font-lock-constant-face)
        (list dcl-label-regexp                   0 'header-line)
        (list dcl-comment-regexp                 0 'font-lock-comment-face)
        (list dcl-special-symbol-regexp          0 'font-lock-negation-char-face))
  "Describes how to syntax highlight keywords in `dcl-mode' buffers.")

(defvar dcl-font-lock-keywords-OLD
  '(("\\<\\(if\\|then\\|else\\|endif\\)\\>"
...
(defconst dcl-font-lock-defaults
  '(dcl-font-lock-keywords nil t)
...
"
  ...
  ;; font lock (case insesnsitive)
  (setq-local font-lock-defaults '(dcl-font-lock-keywords nil t))
